...
def send_payment_confirmation_email(payment_obj, gift):
    ...
    try:
        ...
        html_content = render_to_string(
            "home/emails/payment_confirmation.html", context
        )
        ...
        logger.info(
            f"E-mail de confirmação enviado para {to_email} - Pagamento: {payment_obj.payment_id}"
        )
        return True
...
def send_payment_pending_email(payment_obj, gift):
    ...
    try:
        ...
        logger.info(
            f"E-mail de pendência enviado para {to_email} - Pagamento: {payment_obj.payment_id}"
        )
        return True
...
def get_friendly_payment_message(status_detail, status=None):
    ...
    if status == "approved":
        return approved_messages.get(
            status_detail, "Pagamento aprovado com sucesso! 🎉"
        )
...
class IndexView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["leave_a_message_text"] = TextContent.objects.filter(
            position="leave_a_message_text"
        ).first()
        context["leave_a_message_text2"] = TextContent.objects.filter(
            position="leave_a_message_text2"
        ).first()
        context["last_text"] = TextContent.objects.filter(position="last_text").first()
        context["featured_circles"] = Gallery.objects.filter(
            featured=True, position="circles"
        ).all()
        context["featured_gallery"] = Gallery.objects.filter(
            featured=True, position="gallery"
        ).all()
        return context
...
class AboutUsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["about_us_text_1"] = TextContent.objects.filter(
            position="about_us_text_1"
        ).first()
        context["about_us_text_2"] = TextContent.objects.filter(
            position="about_us_text_2"
        ).first()
        context["about_us_text_3"] = TextContent.objects.filter(
            position="about_us_text_3"
        ).first()
        context["about_us_text_4"] = TextContent.objects.filter(
            position="about_us_text_4"
        ).first()
        context["about_us_text_5"] = TextContent.objects.filter(
            position="about_us_text_5"
        ).first()
        return context
...
class GalleryView(UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["gallery_text_1"] = TextContent.objects.filter(
            position="gallery_text_1"
        ).first()
        context["gallery_text_2"] = TextContent.objects.filter(
            position="gallery_text_2"
        ).first()
...
class Payload:
    def __init__(self, nome, chavepix, valor, cidade, txtId, diretorio=""):
        ...
        self.merchantAccount_tam = (
            f"0014br.gov.bcb.pix01{self.chavepix_tam:02d}{self.chavepix}"
        )

        self.transactionAmount_tam = (
            f'{len(f"{float(self.valor):.2f}"):02d}{float(self.valor):.2f}'
        )
        ...
        self.merchantAccount = (
            f"26{len(self.merchantAccount_tam):02d}{self.merchantAccount_tam}"
        )
    ...
    def _formatar_chave_pix(self, chave):
        ...
        if (
            chave_limpa.startswith("55")
            and chave_limpa.isdigit()
            and len(chave_limpa) == 13
        ):
    ...
    def gerarCrc16(self, payload):
        ...
        self.crc16Code_formatado = (
            str(self.crc16Code).replace("0x", "").upper().zfill(4)
        )
...
class GiftListView(UserPassesTestMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["gift_list_text"] = TextContent.objects.filter(
            position="gift_list_text"
        ).first()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "check_phone":
            ...
            guest = Guest.objects.create(
                name=name, phone=phone_number, self_created=True
            )
            ...
            try:
                ...
                return JsonResponse(
                    {"error": "Multiple guests found with the same name and phone"},
                    status=400,
                )
...
class BridalShowerGiftListView(UserPassesTestMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["bridal_shower_text"] = TextContent.objects.filter(
            position="bridal_shower_text"
        ).first()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if way_to_gift == "money":
            messages.success(
                request, f"Presente {gift.name} escolhido com sucesso! Confira o pix"
            )
            return HttpResponseRedirect(
                reverse("home:bridal_shower_gift_list")
                + f"?phone={guest_phone}&email={guest_email}&gift={gift_id}"
            )
        else:
            messages.success(request, f"Presente {gift.name} escolhido com sucesso!")
            return HttpResponseRedirect(
                reverse("home:bridal_shower_gift_list")
                + f"?phone={guest_phone}&email={guest_email}"
            )
    ...
    def post(self, request):
        try:
            ...
            if not payment_method_id:
                try:
                    ...
                    if card_info and "response" in card_info:
                        payment_method_id = card_info["response"].get(
                            "payment_method_id"
                        )
                        if not issuer_id:
            ...
            if "id" not in payment:
                ...
                return JsonResponse(
                    {"status": "error", "message": error_message, "details": payment},
                    status=400,
                )
            ...
            Payment.objects.create(
                ...
                installments=int(data.get("installments", 1)),
                payment_date=(
                    datetime.now() if payment["status"] == "approved" else None
                ),
            )
                ...
                try:
                    ...
                    if new_status == "approved":
                        try:
                            ...
                            logger.error(
                                f"Erro ao enviar e-mail de confirmação: {str(email_error)}"
                            )
...
class PaymentFailureView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if error_raw:
            ...
            context["error_message"] = get_friendly_payment_message(
                error_raw, "rejected"
            )
            context["suggestion"] = get_payment_suggestion(error_raw)
        else:
            context["error_message"] = (
                error_raw if error_raw else "Não foi possível processar o pagamento."
            )
            context["suggestion"] = (
                "Verifique os dados do cartão ou tente com outro método de pagamento."
            )
